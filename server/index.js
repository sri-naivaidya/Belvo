...
app.get("/api/health", async (req, res) => {
  ...
  res.json({
    success: true,
    db: ready ? "connected" : "not configured",
...
app.post("/api/auth/login", async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      { userId: profile._id.toString(), email: profile.email, role: profile.role || "client" },
      PORTAL_JWT_SECRET,
      { expiresIn: "7d" }
    );
    ...
    res.json({
      success: true,
      user: { id: profile._id.toString(), email: profile.email, fullName: profile.full_name, role: profile.role || "client" },
    });
  } catch (err) {
...
app.post("/api/auth/signup", async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      { userId: profile._id.toString(), email: profile.email, role: profile.role },
      PORTAL_JWT_SECRET,
      { expiresIn: "7d" }
    );
    ...
    res.status(201).json({
      success: true,
      user: { id: profile._id.toString(), email: profile.email, fullName: profile.full_name, role: profile.role },
    });
  } catch (err) {
...
app.get("/api/client/dashboard", authenticatePortal, async (req, res) => {
  try {
    ...
    const timelineEvents = await db
      ...
      .find({ client_id: clientId, visible_to_client: true })
      .sort({ event_date: -1 })
      .limit(5)
      .toArray();
...
// POST /api/register — Save event registration
...
// POST /api/book-call — Save contact/career submissions to MongoDB
...
if (!process.env.VERCEL) {
  app.listen(PORT, async () => {
    ...
    console.log(`  ✦ Database: ${ready ? "✅ Connected" : "⚠️  Not configured (set MONGODB_URI env var)"}`);
