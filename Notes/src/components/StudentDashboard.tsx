...
import { BookOpen, Award, TrendingUp, AlertCircle, CheckCircle, Clock, Play, GraduationCap, ArrowRight, BrainCircuit, Target, ListTodo, FileText } from 'lucide-react';
    ...
    <div className="space-y-6 font-sans text-text-app">
      <div className="p-6 bg-brand rounded-2xl text-white flex flex-col md:flex-row md:items-center justify-between gap-4 border border-border-app shadow-xl">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button onClick={onOpenPdfNotes}
            className="px-4 py-2 bg-bg-card hover:bg-bg-panel text-text-app text-xs font-bold rounded-xl transition flex items-center gap-2 cursor-pointer border border-border-app group">
            <FileText className="w-4 h-4 text-brand" /> PDF Notes <ArrowRight className="w-3 h-3 group-hover:translate-x-0.5 transition-transform" />
          </button>
          {activeOngoingTest ? (
